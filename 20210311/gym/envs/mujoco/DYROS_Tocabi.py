...
class DYROSTocabiEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        qpos = self.sim.data.qpos
        qvel = self.sim.data.qvel
        qpos_urdf = np.zeros_like(qpos)
        qpos_urdf[0:3] = qpos[0:3]
        qpos_urdf[3:6] = qpos[4:7]
        qpos_urdf[6:12] = qpos[7:13]
        qpos_urdf[12:18] = qpos[13:19]
        qpos_urdf[18:21] = qpos[19:22]
        qpos_urdf[21:29] = qpos[24:32]
        qpos_urdf[29:31] = qpos[22:24]
        qpos_urdf[31:39] = qpos[32:40]
        qpos_urdf[39] = qpos[3]
        self.robot.updateDynamics(qpos_urdf)
        torque_graviry_urdf = self.robot.get_gravity_compensation()
        torque_gravity= np.copy(torque_graviry_urdf)
        torque_gravity[0:6] = torque_graviry_urdf[0:6]
        torque_gravity[6:12] = torque_graviry_urdf[6:12]
        torque_gravity[12:15] = torque_graviry_urdf[12:15]
        torque_gravity[15:17] = torque_graviry_urdf[23:25]
        torque_gravity[17:25] = torque_graviry_urdf[15:23]
        torque_gravity[25:33] = torque_graviry_urdf[25:33]
 
        for i in range(self.frame_skip):
            qpos = self.sim.data.qpos
            qvel = self.sim.data.qvel
            torque = torque_gravity + 0.0*(target_data_qpos + a - qpos[7:]) + 0.0*(- qvel[6:])
        ...
        mimic_qpos_reward = 0.0 # 0.4 * exp(-2.0*(np.linalg.norm((qpos_weight*(target_data_qpos - qpos.flat[7:]))**2).mean()))
        mimic_qvel_reward = 0.0 # 0.00 * exp(-0.1*(np.linalg.norm(target_data_qvel - qvel.flat[6:])**2))
        mimic_body_reward = 0.0 # 0.2 * exp(-10*(np.linalg.norm(Tar_Body - qpos.flat[0:3])**2)) 
        mimic_body_orientation_reward = 0.0 # 0.1 * exp(-200*baseQuatError)
        mimic_body_vel_reward = 0.0 # 0.1*exp(-5.0*(np.linalg.norm(target_data_body_vel - qvel.flat[0:3])**2)) # 
    ...
    def reset_model(self):
        ...
        q_desired[27] = -3.14/2
        q_desired[28] = 3.14/2
        print(self.init_qpos + np.concatenate((np.zeros(7), q_desired)))
        self.action_buffer = np.zeros([10,self.action_space.sample().shape[0]])
        self.buffer_idx = 0
