...
class DYROSRedEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        if(self.mocap_data_idx >= self.init_mocap_data_idx):
            ...
            target_data_body_delta[0] = cubic(local_time, self.mocap_data[23,0] + self.mocap_data[self.mocap_data_idx,0], self.mocap_data[23,0] + self.mocap_data[next_idx,0], self.mocap_data[23,1] + self.mocap_data[self.mocap_data_idx,1] - self.mocap_data[self.init_mocap_data_idx,1], self.mocap_data[23,1] + self.mocap_data[next_idx,1] - self.mocap_data[self.init_mocap_data_idx,1], 0.0, 0.0)
            target_data_body_delta[1] = cubic(local_time, self.mocap_data[23,0] + self.mocap_data[self.mocap_data_idx,0], self.mocap_data[23,0] + self.mocap_data[next_idx,0], self.mocap_data[self.mocap_data_idx,3] - self.mocap_data[self.init_mocap_data_idx,3], self.mocap_data[next_idx,3] - self.mocap_data[self.init_mocap_data_idx,3], 0.0, 0.0)
            target_data_body_delta[2] = cubic(local_time, self.mocap_data[23,0] + self.mocap_data[self.mocap_data_idx,0], self.mocap_data[23,0] + self.mocap_data[next_idx,0], self.mocap_data[self.mocap_data_idx,2] - self.mocap_data[self.init_mocap_data_idx,2], self.mocap_data[next_idx,2] - self.mocap_data[self.init_mocap_data_idx,2], 0.0, 0.0)
        ...
        basequat_desired = Quaternion([1,0,0,0]) #Quaternion(self.mocap_data[self.mocap_data_idx,4:8])
        ...
        if (self.mocap_data_idx == 9 or self.mocap_data_idx == 10 or self.mocap_data_idx == 21 or self.mocap_data_idx == 22):
            if (self.r_contact is False and self.l_contact is False):
                mimic_contact_reward = 0.2
            else:
                mimic_contact_reward = 0.0
        elif (self.mocap_data_idx <= 8 or self.mocap_data_idx >= 23):
            if (self.r_contact is True and self.l_contact is False):
                mimic_contact_reward = 0.2
            else:
                mimic_contact_reward = 0.0
        elif (self.mocap_data_idx >= 11 and self.mocap_data_idx <= 20):
            if (self.r_contact is False and self.l_contact is True):
                mimic_contact_reward = 0.2
            else:
                mimic_contact_reward = 0.0

        mimic_qpos_reward = 0.45 * exp(-2.0*(np.linalg.norm((target_data_qpos - qpos.flat[7:])**2).mean()))
        mimic_qvel_reward = 0.00 * exp(-0.1*(np.linalg.norm(target_data_qvel - qvel.flat[6:])**2))
        mimic_ee_reward = 0.0 * exp(-40*(np.linalg.norm(EE_CoM - Tar_EE_COM.flatten())**2))
        mimic_body_reward = 0.25 * exp(-10*(np.linalg.norm(Tar_Body - qpos.flat[0:3])**2 + 0.5*baseQuatError**2)) 
        ...
        if not done_by_contact:
            ...
            return self._get_obs(), reward, done_by_contact, dict(specific_reward=dict(mimic_qpos_reward=mimic_qpos_reward, mimic_qvel_reward=mimic_qvel_reward, mimic_ee_reward= mimic_ee_reward, mimic_body_reward=mimic_body_reward, mimic_body_vel_reward=mimic_body_vel_reward, mimic_contact_reward=mimic_contact_reward))
        else:
    ...
    def reset_model(self):
        ...
        self.epi_len = 0
        self.epi_reward = 0
        self.init_mocap_data_idx = np.random.randint(low=0, high=23)
